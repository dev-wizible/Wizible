      ...
      try {
        ...
        const response = await this.openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are an expert HR recruiter and resume evaluator. You must respond with valid JSON only, no additional text or markdown formatting.'
            },
            {
              role: 'user',
              content: prompt
...
`.trim();
  ...
  private validateScores(scores: any): void {
    const required = ['skills_match_score', 'experience_score', 'overall_score', 'reasoning'];
    ...
    const scoreFields = ['skills_match_score', 'experience_score', 'overall_score'];
    for (const field of scoreFields) {
      const score = scores[field];
      if (typeof score !== 'number' || score < 0 || score > 100) {
        throw new Error(`Invalid score for ${field}: must be a number between 0-100`);
      }
    }
